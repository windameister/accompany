#!/usr/bin/env python3
"""
Persistent MLX TTS/STT server. Models loaded once, stay in memory.
...
def warmup_tts():
    ...
    generate_audio(
        ...
        file_prefix=f"_warmup_{os.getpid()}", verbose=False,
    )
    # Cleanup warmup file
    for f in os.listdir("/tmp"):
        if f.startswith(f"_warmup_{os.getpid()}"):
...
def warmup_stt():
    ...
    tmp = f"/tmp/_stt_warmup_{os.getpid()}.wav"
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/health":
            self._json({"status": "ok", "tts": tts_ready, "stt": stt_ready})
        else:
    ...
    def do_POST(self):
        ...
        if self.path == "/tts":
            ...
            if not text:
                self._json({"status": "error", "message": "no text"}, 400); return
